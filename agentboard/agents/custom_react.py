...
@registry.register_agent("CustomReactAgent")
class CustomReactAgent(
    ...
    def make_prompt(self,
        ...
        # print(f'Number of tokens calculated {num_of_tokens}.')
        while num_of_tokens > self.max_context_length - self.llm_model.max_tokens - 100:
            history = history[1:]
            input_prompt = query + "\n".join([item[0] + ": " + item[1] for item in history])
            ...
            #     input_prompt +="\nPlease stop thinking and start taking action on the task."
            if self.force_action:
                input_prompt += "\nYou MUST take action on the task."
    ...
    def run(self, init_prompt_dict=None):
        ...
        max_retry, cnt = 5, 0
        while not flag and cnt < max_retry:
            ...
            if 'action' in response.lower():
                ...
                print("Step {:02} - Think: {}".format(self.steps, think))
                self.memory.append(
                    ("Think", think)
                )
                action = self.extract_action(response)
                # print("Step {:02} - Action: {}".format(self.steps, action))
                if self.use_parser:
                ...
                print("Step {:02} - Response: {}".format(self.steps, response))
