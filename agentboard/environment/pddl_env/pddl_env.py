...
class PDDL(BaseEnvironment):
    ...
    def special_parser_for_blocks_medium(self, tokens):
        new_tokens = []
        for token in tokens:
            if token.lower() == 'pickup':
                token = 'pick-up'
            elif token.lower() == 'putdown':
                token = 'put-down'
            else:
                pass
    ...
    def text_to_action(self, text):
        ...
        if 'medium' in self.game_name:
            tokens = self.special_parser_for_blocks_medium(tokens)
        predicate_name = None   
        for token in tokens:
...
predicate_map = {  
    # Further added for blocks_medium
    "handfull": "Your arm is full.",
    "handempty": "Your arm is empty.",
    "pick-up": "Pickup {}.",
    "put-down": "Putdown {}.",
    # Blocks
    "on": "{} is on {}.",
